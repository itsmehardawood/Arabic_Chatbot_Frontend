...
export default function ChatContainer() {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const url = `https://itsmehardawood-arabic-chatbot.hf.space/build_rag?user_id=${encodeURIComponent(user_id)}`
  ...
  const handleSendMessage = async ({ question, level, diacritics }) => {
    ...
    try {
      ...
      const res = await fetch('https://itsmehardawood-arabic-chatbot.hf.space/query_rag', {
        method: 'POST',
