...
// Function to detect language and get appropriate voice
const getVoiceForText = (text) => {
  const voices = window.speechSynthesis.getVoices();
  
  // Simple language detection based on character patterns
  const hasArabic = /[\u0600-\u06FF]/.test(text);
  const hasGerman = /[äöüßÄÖÜ]/.test(text) || /\b(und|oder|der|die|das|ich|bin|ist|sind)\b/i.test(text);
  
  // Priority: Arabic > German > English (default)
  if (hasArabic) {
  ...
  // Default to English or first available voice
...
export default function MessageBubble({ 
  ...
  const handleSpeak = () => {
    ...
    // Auto-detect and set appropriate voice
    ...
    // Apply rate and pitch settings (with sensible defaults)
    utterance.rate = speechSettings.rate || 0.9; // Slightly slower for better clarity
